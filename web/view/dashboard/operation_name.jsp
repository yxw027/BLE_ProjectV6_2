<%-- 
    Document   : dash
    Created on : Feb 04, 2021, 11:46:09 AM
    Author     : saini
--%>
...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <head>
        ...
        <script>
            ...
            $(function () {                
                $("#parent_operation_name").autocomplete({
                    source: function (request, response) {
                        var random = document.getElementById("parent_operation_name").value;
                        $.ajax({
                            url: "OperationNameController",
                            dataType: "json",
                            data: {
                                action1: "getParentOperationName",
                                str: random
                            },
                    ...
                    select: function (events, ui) {
                        console.log(ui);
                        $('#parent_operation_name').val(ui.item.label); // display the selected text
                        return false;
                    }
                });                               
            ...
            function makeEditable(id) {
                ...
                document.getElementById("parent_operation_name").disabled = false;
    ...
    <body onload="zoom();" style="background-color: rgb(56, 165, 238, 0.5);">
        ...
        <div id="page-inner" style="background-color: rgb(56, 165, 238, 0.02);">
                                ...
                                <table class="table table-striped table-bordered table-hover"
                                    ...
                                    <tbody>
                                        <c:forEach var="model" items="${requestScope['list']}"
                                            ...
                                            <tr
                                                onclick="fillColumn('${model.operation_name_id}');">
                                                <td>${loopCounter.count }</td>
                                                <td id="${loopCounter.count }2">${model.parent_name}</td>
                                                <td id="${loopCounter.count }3">${model.operation_name}</td>
                                                <td id="${loopCounter.count }4">${model.is_super_child}</td>                                                                                             
                                                <td id="${loopCounter.count }5">${model.generation}</td>                                                                                             
                                                <td id="${loopCounter.count }6">${model.remark}</td>                                                                                                
                                            </tr>
                                        </c:forEach>
                                    </tbody>
                                </table>
            ...
            <div class="row" style="margin-top: 10px;margin-left: 10%;margin-right: 10%;font-size:14px ;padding:0px 15px 0px 15px" id="">
                <form method="post" action="OperationNameController" name="form1" style="background-color: white;">                    
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-5" style="margin-top:35px">                            
                            <input type="text" disabled name="operation_name" id="operation_name" class="form-control m-input" placeholder="Enter Operation Name" required>
                        </div>                        
                    </div>

                    <div class="row">
                        <div class="col-lg-5 col_label">
                            <label class="required" style="font-size:20px;">Parent Operation Name:</label>
                        </div>
                        <div class="col-lg-5">                            
                            <input type="text" disabled name="parent_operation_name" id="parent_operation_name" class="form-control m-input" placeholder="Enter Parent Operation Name">
                        </div>                        
                    </div>
