...
import com.ble.response.model.ResponseSubByteDivisionModel;
import com.ble.response.model.ResponseSubDivisionSelectionModel;
 ...
 * @author user
 */
public class ResponseSubByteDivisionController extends HttpServlet {
     ...
     protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ResponseSubByteDivisionModel subByteDivisionModel = new  ResponseSubByteDivisionModel();
        ...
        int response_sub_byte_division_id;
        ...
        String byte_data_response_id = request.getParameter("byte_data_response_id");
...
//        List<SelectionBean> selectionList = selectionModel.showData(lowerLimit, noOfRowsToDisplay, searchCommandName);
        if (task.equals("Save") || task.equals("Save AS New")) {
            ...
            try {
                response_sub_byte_division_id = Integer.parseInt(request.getParameter("response_sub_byte_division_id"));
            } catch (Exception e) {
              response_sub_byte_division_id = 0;
            }
            ...
            if (sub_byte_division != null && parameter_name_byte1 != null) {
               int selection =Integer.parseInt(sub_byte_division);
                for (int i = 1; i <= selection; i++) {
                    ResponseSubByteDivisionBean bean = new ResponseSubByteDivisionBean();
                    bean.setResponse_sub_byte_division_id(response_sub_byte_division_id);
                    ...
                    if (response_sub_byte_division_id == 0) {
        ...
        if (task.equals("update")) {
            ...
            response_sub_byte_division_id = Integer.parseInt(request.getParameter("response_sub_byte_division_id"+count));
            int sub_division_no = Integer.parseInt(request.getParameter("sub_division_no"+count));
          
            byte_data_response_id = request.getParameter("byte_data_response_id");
            ResponseSubByteDivisionBean bean = new  ResponseSubByteDivisionBean();
            bean.setResponse_sub_byte_division_id(response_sub_byte_division_id);
        ...
        if (sub_byte_division != null) {
            ...
            String parameter_name_byte = subByteDivisionModel.getParameterNameByByte_id(Integer.parseInt(byte_data_response_id));
            int length = parameter_name_byte.length();
            request.setAttribute("parameter_name", parameter_name_byte);
            request.setAttribute("byte_data_response_id", byte_data_response_id);
            List<ResponseSubByteDivisionBean> sub_byte_divisionListById =  subByteDivisionModel.showDataByByteId(lowerLimit, noOfRowsToDisplay, sub_byte_division, byte_data_response_id);
            request.setAttribute("sub_byte_divisionListById", sub_byte_divisionListById);
            request.getRequestDispatcher("/response_sub_byte_division").forward(request, response);
        }
