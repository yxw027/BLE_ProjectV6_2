...
import com.ble.dataEntry.model.BleOperationNameModel;
 ...
 * @author jpss
 */
public class BleOperationNameController extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        BleOperationNameModel ruleModel = new BleOperationNameModel();
...
//            }
         if (task.equals("Cancel")) {
            ruleModel.deleteRecord(Integer.parseInt(request.getParameter("ble_operation_name_id")));  // Pretty sure that organisation_type_id will be available.
        } else if (task.equals("Save") || task.equals("Save AS New")) {
            int ble_operation_name_id;
            try {
                ble_operation_name_id = Integer.parseInt(request.getParameter("ble_operation_name_id"));
            } catch (Exception e) {
                ble_operation_name_id = 0;
            }
            if (task.equals("Save AS New")) {
                ble_operation_name_id = 0;
            }
            BleOperationNameBean ruleBean = new BleOperationNameBean();
            ruleBean.setBle_operation_name_id(ble_operation_name_id);
            ruleBean.setBle_operation_name(request.getParameter("ble_operation_name"));
            ruleBean.setRemark(request.getParameter("remark"));

            if (ble_operation_name_id == 0) {
        ...
        List<BleOperationNameBean> commandTypeList = ruleModel.showData(lowerLimit, noOfRowsToDisplay,searchCommandName);
        ...
        request.getRequestDispatcher("/ble_operation_name").forward(request, response);
    }
