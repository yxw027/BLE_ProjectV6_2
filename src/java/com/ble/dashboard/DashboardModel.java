...
public class DashboardModel {
    ...
    public List<DashboardBean> getModulesList(String q) {
        List<DashboardBean> list = new ArrayList<DashboardBean>();
        try {
            String qry = " select  distinct m.device_name,d.id,i.image_id,i.image_path "
            ...
            while (rset.next()) {
                ...
                dashBean.setImage_id(rset.getString(3));
                dashBean.setImage_path(rset.getString(4));
                list.add(dashBean);
            }
        } catch (Exception e) {
            System.out.println("getModulesList ERROR inside DashboardModel - " + e);
        }
        return list;
    }
...
//                    + " and mt.active='Y' and m.model_type_id=mt.id order by dm.finished_device_id ";
            String query = " select  distinct m.device_name,d.id,i.image_path "
                    + " from device_type dt, device d, model m,image i "
                    + " where dt.active='Y' and d.active='Y' and m.active='Y' and dt.type ='Finished' and "
                    + " dt.id=d.device_type_id and d.model_id=m.id and d.image_id=i.image_id ";
    ...
    public List<DashboardBean> getOperationList(String q) {
        ...
        //String[] ids = device_id.split("\\s*,\\s*");        
        try {
            String qry = " select * from operation_name where active='Y' ";
            PreparedStatement pst = connection.prepareStatement(qry);
            ResultSet rset = pst.executeQuery();
            while (rset.next()) {
                DashboardBean deviceBean = new DashboardBean();
            ...
            System.out.println("getOperationList ERROR inside DashboardModel - " + e);
        }
        return list;
    }
    ...
    public List<DashboardBean> getCommandList(String q) {
        ...
        //String[] ids = device_id.split("\\s*,\\s*");        
        try {
            String qry = " select * from command where active='Y' ";
            PreparedStatement pst = connection.prepareStatement(qry);
            ResultSet rset = pst.executeQuery();
            while (rset.next()) {
                DashboardBean deviceBean = new DashboardBean();
                deviceBean.setCommand(rset.getString(1));
                list.add(deviceBean);
            }
        } catch (Exception e) {
            System.out.println("getCommandList ERROR inside DashboardModel - " + e);
        }
        return list;
    }
    ...
    public List<DashboardBean> getDevOpCommandList(String q) {
        ...
        try {
            ...
            while (rset.next()) {
                DashboardBean deviceBean = new DashboardBean();
                deviceBean.setDevice_name(rset.getString(1));
                list.add(deviceBean);
            }
        } catch (Exception e) {
            System.out.println("getDevOpCommandList ERROR inside DashboardModel - " + e);
        }
        return list;
    }
    ...
    public List<DashboardBean> getParameterList(String q) {
        ...
        //String[] ids = device_id.split("\\s*,\\s*");        
        try {
            String qry = " select * from parameter where active='Y' ";
            PreparedStatement pst = connection.prepareStatement(qry);
            ResultSet rset = pst.executeQuery();
            while (rset.next()) {
                DashboardBean deviceBean = new DashboardBean();
            ...
            System.out.println("getParameterList ERROR inside DashboardModel - " + e);
        }
        return list;
    }
