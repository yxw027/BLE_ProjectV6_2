...
public class ManufacturerModel {
    ...
    public List<ManufacturerBean> showData() {
        List<ManufacturerBean> list = new ArrayList<ManufacturerBean>();
        PreparedStatement psmt;
        ResultSet rst;
        String query = "";
        try {
            query = " select * from manufacturer order by id desc ";
            psmt = connection.prepareStatement(query);
            rst = psmt.executeQuery();
            while (rst.next()) {
                ManufacturerBean bean = new ManufacturerBean();
                bean.setManufacturer_id(rst.getString(1));
                bean.setManufacturer_name(rst.getString(2));
            ...
            System.out.println("com.ble.dashboard.dataEntry.ManufacturerModel.showdata() -" + e);
        }
        return list;
    }
        

    public int insertRecord(ManufacturerBean bean) throws SQLException {            
        String query="";
        PreparedStatement psmt;
        int count=0;
        try {            
            connection.setAutoCommit(false);
            query=" insert into manufacturer(name,remark) values(?,?)";
            psmt = connection.prepareStatement(query);
            psmt.setString(1, bean.getManufacturer_name());
